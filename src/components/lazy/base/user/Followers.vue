    ...
    >
      <span class="p-input-icon-left">
        ...
        <p-input-text
          ...
          :placeholder="$t('search.search_followers')"
          @input="search"
        />
      </span>
    </div>
      ...
      <div class="followers">
        <div v-for="(item, index) in data.content" :key="index" class="item">
          <UserListItem
            ...
            @click="gotoProfile(lodash.get(item, 'subscriber.id'))"
            :displayText="lodash.get(item, 'subscriber.displayName')"
            :assiveText="location(lodash.get(item, 'subscriber.cityId'))"
            :avatar="lodash.get(item, 'subscriber.avatar.path')"
          >
            <template #button>
              <InviteButton v-if="type == 'invite'" :person-id="lodash.get(item, 'subscriber.id')" />
              <MessageButton
                v-else
                :person-id="lodash.get(item, 'subscriber.id')"
...
const fetch = async () => {
  ...
  data.value = await personStore.getFollowers(userId, params)
  loading.value = false
}
